@include('layouts.header')
  ...
  <main id="main">
    <section class="profile_provider">
      <div class="container">
        @php
        $provider_list = DB::table('provider_listing')
                 ->join('users','provider_listing.provider_id', "=" ,'users.id')
                 ->where('provider_listing.provider_id',$provider->id)->get();
        ...
        <div class="profile_head" style="background-image: url('{{asset('images/banner-group.jpg')}}');">
          ...
          <div class="rating_bar">
            <ul>
              <li>
                <span>{{$average_rating}} <i class="fa-solid fa-star"></i></span>
                Ratings
              </li>
              <li>
        ...
        <div class="profile_middle">
          <div class="profile_detail">
            <div class="profile">
              <figure>
                <img src="{{asset('uploads/provider/'.$prov->provider_id.'/business_profile/'.$prov->image)}}" alt="profile">
              </figure>
              <div class="ctnt">
                <h5>{{$provider->name.' '.$provider->lastname}}
                ...
                <p style="color: #47BBD0;">Online</p>
                <!-- <span><i class="fa-solid fa-font-awesome"></i> Report this member</span> -->
              </div>
            </div>
            <div class="pro_btn">
              <a href="/provider-edit-profile/{{$provider->id}}" class="btn active"><i class="fa-solid fa-pen"></i> Edit</a>
            </div>
          </div>
          <div class="wraper_row">
                ...
                <div class="badge_ctnt">
                  ...
                  <ul>
                    <li>
                      ...
                      @php
                      $identity_document = json_decode($prov->identity_document, true);
                      $address_proof = json_decode($prov->address_proof, true);
                      $bus_registration = json_decode($prov->bus_registration, true);
                      $police_cer = json_decode($prov->police_cer, true);
                      @endphp
                      <div class="ctnt">
                        Police Certificate
                        @if(isset($police_cer))
                        @if($police_cer['status'] == 0)
                        <span>Verified</span>
                        @else
                        <span>Pending</span>
                        @endif
                  ...
                  <div class="about_ctnt">
                    ...
                    <p>{{$prov->n_employees}}</p>
                    <div class="gallery">
                      <div class="gallery_head">
                        <h5>Gallery <span class="badge">{{count($pro)}}</span></h5>
                      </div>
                      <div class="row">
                        
                        @foreach($pro as $bimage)
                        <div class="col-md-4">
                          <div class="gallery_ctnt">
                           <figure style="background-image: url('{{asset('uploads/provider/'.$provider->id.'/business/'.$bimage)}}');"></figure>
                          </div>
                        </div>
                        @endforeach
                    ...
                    @php
                    $ser_lists = DB::table('listed_services')->where('lis_providerid',$provider->id)->where('status',0)->get();
                    $coun_ser = count($ser_lists);
                     
                    @endphp
                    <div class="service_list">
                      <div class="gallery_head">

                        <h5>Service List <span class="badge">{{$coun_ser}}</span></h5>
                      </div>
                      <div class="row">
                        @foreach($ser_lists as $ser_list)
                        <div class="col-md-4">
                          <div class="gallery_ctnt">
                            <figure style="background-image: url('{{asset('uploads/provider/'.$ser_list->lis_providerid.'/listing_image/'.$ser_list->service_image)}}')"></figure>
                            <h5>{{ucfirst($ser_list->service_name)}}</h5>
                            <p>From ${{$prov->price_per_hour}} hr</p>
                    ...
                    <div class="rating_list">
                      <div class="gallery_head">
                        <h5>Total Review <span class="badge">{{$count_review}}</span></h5>
                      </div>
                      @php $rating = $average_rating; @endphp
                      <div class="rating_ctnt">
                          ...
                          <div class="rating_star">
                            ...
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0"
                                aria-valuemax="100" style="width:<?php if($count_review > 0){ echo $star_5/$count_review*100;}else{echo 0;} ?>%">
                          ...
                          <div class="rating_star">
                            ...
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="32" aria-valuemin="0"
                                aria-valuemax="100" style="width:<?php if($count_review > 0){ echo $star_4/$count_review*100;}else{echo 0;} ?>%">
                          ...
                          <div class="rating_star">
                            ...
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="60" aria-valuemin="0"
                                aria-valuemax="100" style="width:<?php if($count_review > 0){ echo $star_3/$count_review*100;}else{echo 0;} ?>%">
                          ...
                          <div class="rating_star">
                            ...
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="35" aria-valuemin="0"
                                aria-valuemax="100" style="width:<?php if($count_review > 0){ echo $star_2/$count_review*100;}else{echo 0;} ?>%">
                          ...
                          <div class="rating_star">
                            ...
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="20" aria-valuemin="0"
                                aria-valuemax="100" style="width:<?php if($count_review > 0){ echo $star_1/$count_review*100;}else{echo 0;} ?>%">
                       ...
                       @foreach($review_comment as $review_comm)
                      <div class="author">
                        <div class="lft_ctnt">
                          @php
                          $user_img = DB::table('users')->select('image')->where('id',$review_comm->user_id)->first();
                          $task_category = DB::table('tsk_detail')->select('cleaning_radio')->where('id',$review_comm->task_id)->first();
                          @endphp
                          <figure style="background-color:transparent;">
                            <img src="{{asset('uploads/user/'.$review_comm->user_id.'/'.$user_img->image)}}">
                          </figure>
                        </div>
                        <div class="author_right">
                          <div class="head_main">
                            <div class="main_lft">
                              @php $rat = $review_comm->starrating; @endphp  
                              <ul>
                                @foreach(range(1,5) as $i)
                                <span class="fa-stack" style="width:1em">
                                  <i class="far fa-star fa-stack-1x" style="color:#b7b7b7"></i>
                              ...
                              <h6>{{$task_category->cleaning_radio}}</h6>
                            </div>
                            <span>{{date("M-d-Y", strtotime($review_comm->created_at)) }}</span>
                          </div>
                          <div class="middle">
                            <p>{{$review_comm->comment}}</p>
                            
                            <strong>Review by: <span>{{$review_comm->user_id}}</span></strong>
            ...
            <div class="col_wrapr_mini">
              ...
              <div class="footer-col">
                <h6>Follow Us</h6>
                <ul class="social">
                  @if(isset($sociallink->facebook_link))
                  <li><a href="{{url($sociallink->facebook_link)}}" target="_blank"><i class="fa-brands fa-facebook-f"></i></a></li>
                  @endif
                  @if(isset($sociallink->instagram_link))
                  <li><a href="{{url($sociallink->instagram_link)}}" target="_blank"><i class="fa-brands fa-instagram"></i></a></li>
                  @endif
                  @if(isset($sociallink->tiktok_link))
                  <li><a href="{{url($sociallink->tiktok_link)}}" target="_blank"><i class="fa-brands fa-tiktok"></i></a></li>
                  @endif
